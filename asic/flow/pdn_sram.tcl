# Custom PDN config for Top_System_SRAM
# Extends the sky130hd default PDN to add global connections for
# OpenRAM-generated SRAM macro power pins (vdd/gnd, lowercase).

####################################
# global connections (sky130hd defaults)
####################################
...
# OpenRAM sky130_sram_1rw1r_64x256_8 uses lowercase vdd/gnd
add_global_connection -net {VDD} -inst_pattern {.*u_macro.*} -pin_pattern {^vdd$}
add_global_connection -net {VSS} -inst_pattern {.*u_macro.*} -pin_pattern {^gnd$}
...
####################################
# macro grids (standard orientations)
####################################
